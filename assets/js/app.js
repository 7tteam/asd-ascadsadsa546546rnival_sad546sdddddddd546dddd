...
class EbyarApp {
    ...
    async saveGuestUser(guestData) {
        try {
            await fetch('backend/session.php', {
                method: 'POST',
    ...
    async getSubCategories(categoryId) {
        try {
            ...
            const response = await fetch(`backend/directory.php?category=${categoryId}`);
            const data = await response.json();
    ...
    createDirectoryCard(contact) {
        ...
        const imagePath = contact.image ? 
            (contact.image.includes('data/upload/') ? contact.image : `data/upload/${contact.image}`) : 
    ...
    async showContactModal(contactId) {
        try {
            const response = await fetch(`backend/directory.php?contact_id=${contactId}`);
            const data = await response.json();
    ...
    async loadHomeData() {
        ...
        try {
            ...
            const newsResponse = await fetch('backend/news.php');
            const newsData = await newsResponse.json();
            this.renderLatestNews(newsData.posts?.slice(0, 3) || []);
            
            // Load upcoming events (first 3)
            const eventsResponse = await fetch('backend/events.php');
            const eventsData = await eventsResponse.json();
            this.renderUpcomingEvents(eventsData.events?.slice(0, 3) || []);
            
            // Load featured contacts
            const featuredResponse = await fetch('backend/directory.php?featured=true');
